...
# Single top-level user query (Experiment 1 baseline: no query expansion)
USER_QUERY = "Extract missing security verification properties related to TD Partitioning in Intel TDX"

DOMAINS = {
    "memory_isolation":    ["EPT", "SEPT", "GPA", "HPA", "alias", "mapping"],
    "hkid_lifecycle":      ["HKID", "KOT", "KET", "binding", "reuse", "keyid"],
    "cpu_state_isolation": ["VM exit", "VM entry", "register", "XSAVE", "state"],
    "cache_tlb_isolation": ["TLB", "cache", "flush", "invalidation", "shootdown"],
    "dma_isolation":       ["DMA", "IOMMU", "device", "external access"],
    "interrupt_isolation": ["interrupt", "vector", "delivery", "routing"],
}

TECH_MARKERS = {
    "memory_isolation":    ["TDH.MEM.SEPT.ADD", "EPT violation", "shared/private GPA bit",
                             "GPA aliasing check"],
    "hkid_lifecycle":      ["TDH.MNG.KEY.CONFIG", "KOT entry", "KET entry", "HKID reuse window"],
    "cpu_state_isolation": ["TDH.VP.ENTER", "TD-VMCS", "XSAVE state area", "guest register scrubbing"],
    "cache_tlb_isolation": ["TDH.MEM.TRACK", "TLB shootdown", "cache way partitioning",
                             "flush on TD switch"],
    "dma_isolation":       ["IOMMU remapping", "device TLB invalidation", "DMA remap fault"],
    "interrupt_isolation": ["posted interrupt descriptor", "APIC virtualization",
...
def build_prompt(assertions: str, context: str) -> str:
    return f"""
You are a senior Intel TDX formal verification engineer.
 
TASK:
Generate additional potentially missing TD partitioning properties
even if partially related to existing assertions.
Prefer deeper microarchitectural isolation properties.
 
FOCUS AREA:
- L1 / L2 VM isolation
- SEPT / EPT aliasing
- GPA to HPA translation correctness
- VM entry / exit state isolation
- TLB / cache isolation across TDs
- HKID binding correctness
- Control-flow isolation between TD layers
- Memory sharing vs private enforcement
 
RULES:
- DO NOT repeat existing properties
- ONLY generate NEW missing properties
- Output MUST be in English
- Use formal verification style:
  (Confidentiality / Integrity / Isolation properties)
- Every property MUST start with a line "Property Name: <short unique name>"
  (this is required for downstream parsing — do not omit it)
- Include for each property:
  Property Name, Definition, Purpose, Security Impact, Assertion Hint, Intel Reference
...
OUTPUT:
Generate ONLY missing TD Partitioning properties:
"""
# =============================================================================
# STEP 8 — LLM CALL
# =============================================================================
