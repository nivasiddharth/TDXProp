"""
Intel TDX RAG Pipeline — MKTME Security Audit — LLM ONLY + STANDARD RAG (PARA)
================================================================================
Module: MKTME (Multi-Key Total Memory Encryption)

This script bundles TWO experiments in one file:
  Experiment 0 : LLM Only        (--no_rag)  — direct query, no document context
  Experiment 1 : Standard RAG                — single query, similarity search,
                                                paragraph-based chunking

Mirrors the structure/metrics of tdxllmpara.py (SEPT+PAMT LLM-only + Standard
RAG file), retargeted to the MKTME domain (DOMAINS / TECH_MARKERS / prompt
taken from Tdxragmktmeabal.py), with paragraph-based ("para") chunking instead
of the document-aware/section-header chunker.

NOTE on USER_ASSERTIONS: kept IDENTICAL to the SEPT/PAMT assertion list, same
as the uploaded Tdxragmktmeabal.py ("...it still contains the SEPT/PAMT (cP/iP)
assertions, not MKTME-specific ones, per your confirmation to proceed as-is").
Swap this block out if/when you have an MKTME-specific verified-assertions list.

Usage:
    # Standard RAG (Experiment 1)
    python llm_and_standard_rag_mktme.py --pdf_dir ./specs

    # LLM Only (Experiment 0, no documents needed)
    python llm_and_standard_rag_mktme.py --no_rag
    python llm_and_standard_rag_mktme.py --no_rag --query "custom question"
"""
...
VECTORSTORE_DIR      = "./vectorstore_mktme"
METADATA_FILE        = os.path.join(VECTORSTORE_DIR, "index_metadata.json")
CHROMA_COLLECTION    = "intel_tdx_mktme"
CHUNKING_VERSION     = "mktme_para"
...
# NOTE: unchanged from the SEPT/PAMT assertion list — see module docstring above.
...
USER_QUERY = "Extract missing security verification properties related to MKTME in Intel TDX"
...
# =============================================================================
# STEP 2 — CHUNKING STRATEGY — PARAGRAPH-BASED
# =============================================================================
...
def print_coverage(text: str):
    ...
    print("\n" + "=" * 70)
    print("  TDX MKTME COVERAGE REPORT")
    print("=" * 70)
...
def compute_technical_grounding(properties: List[str], retrieved_chunks: List[Dict]) -> Dict:
    """
    SEMANTIC-ALIGNMENT CHECK: targets domain-specific technical vocabulary
    (register names, SEAMCALL leaves, MKTME-specific state names from
    TECH_MARKERS). For every technical marker a property mentions, checks
    whether that exact marker also appears verbatim in the retrieved context.
    """
...
Grounded Sources:
- Cite the exact [SOURCE N] tag(s) from the SPEC CONTEXT below that directly support this property, e.g. [SOURCE 3], [SOURCE 7]
- Use ONLY source numbers that actually appear in the SPEC CONTEXT provided below — never invent a source number
- If NO retrieved source directly supports the property, write "Grounded Sources: NONE — based on general TDX architecture knowledge, verify against full specification before use"
Reference:
- Intel TDX Module Specification / Intel SDM / Intel MKTME documentation
- Mention topic (exact section number not required)

FACTUAL GROUNDING RULES (STRICT — violating these makes a property unusable):
- Every specific technical claim (register/field names, SEAMCALL leaf names, state names, bit positions, opcodes) MUST either (a) appear in the SPEC CONTEXT below, or (b) be well-established, unambiguous TDX/MKTME terminology you are highly confident about
- Do NOT fabricate signal names, SEAMCALL leaf names, or field names that do not appear in the provided context
- Do NOT paraphrase a SPEC CONTEXT passage so loosely that the technical meaning changes
- If the retrieved context only partially supports a property, narrow the property's scope to what IS supported rather than extrapolating
- Prefer fewer, well-grounded properties over many loosely-grounded ones
- Two properties must not overlap in meaning — each must target a semantically distinct verification concern
...
TDX + MKTME SPEC CONTEXT (retrieved from Intel documentation, each block tagged [SOURCE N]):
{context}

OUTPUT:
Generate high-value missing TDX + MKTME verification properties. Every property must include a "Grounded Sources" field as instructed above:
"""
...
# =============================================================================
# EXPERIMENT RUNNER (EXPERIMENT 1: STANDARD RAG)
# =============================================================================

def run_experiment_1(vectorstore, client, output_dir: str = ".") -> Dict:
    name = "standard_rag"
    print(f"\n{'='*70}\n  EXPERIMENT 1: {name} (MKTME)\n{'='*70}")

    metrics: Dict = {"experiment": 1, "name": name, "module": "mktme"}
    ...
    with open(os.path.join(output_dir, "mktme_retrieved_chunks_exp1.json"), "w", encoding="utf-8") as f:
    ...
    # --- Save outputs ---
    out_path = os.path.join(output_dir, "missing_mktme_properties_exp1.txt")
    ...
    annotated_path = os.path.join(output_dir, "missing_mktme_properties_exp1_ANNOTATED.txt")
    with open(annotated_path, "w", encoding="utf-8") as f:
        f.write(annotated_text)
    metrics["annotated_output_file"] = annotated_path

    metrics_path = os.path.join(output_dir, "mktme_metrics_exp1.json")
...
# =============================================================================
# EXPERIMENT RUNNER (EXPERIMENT 0: LLM ONLY / NO-RAG)
# =============================================================================
...
def run_no_rag_query(query: str, client, output_dir: str = ".") -> Dict:
    ...
    """
    print(f"\n{'='*70}\n  LLM ONLY / DIRECT QUERY MODE (No document / No RAG) — MKTME\n{'='*70}")

    metrics: Dict = {"mode": "no_rag_direct_query", "module": "mktme", "model": OPENAI_MODEL, "query": query}
    ...
    out_path = os.path.join(output_dir, "no_rag_mktme_response.txt")
    with open(out_path, "w", encoding="utf-8") as f:
        f.write(result)
    metrics["output_file"] = out_path

    # --- Save a standalone coverage report ---
    coverage_report_lines = [
        "=" * 70,
        "  TDX MKTME COVERAGE REPORT (NO-RAG / DIRECT QUERY)",
        "=" * 70,
    ]
    ...
    coverage_report_path = os.path.join(output_dir, "no_rag_mktme_coverage_report.txt")
    ...
    metrics_path = os.path.join(output_dir, "no_rag_mktme_metrics.json")
    with open(metrics_path, "w", encoding="utf-8") as f:
        json.dump(metrics, f, indent=2)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Intel TDX RAG — MKTME — LLM Only + Standard RAG")
    ...
    parser.add_argument("--query", metavar="TEXT",
                         help="Question to ask in --no_rag mode (defaults to the standard MKTME query if omitted)")
